...
const createUser = async (requestBody: any) => {
    try {
        ...
        throw new InternalError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: userService.createUser - ${getErrorMessage(error)}`)
    }
}
...
const deleteUser = async (userId: string): Promise<any> => {
    try {
        ...
        throw new InternalError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: userService.deleteUser - ${getErrorMessage(error)}`)
    }
}
...
const getAllUsers = async () => {
    try {
        ...
        throw new InternalError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: userService.getAllUsers - ${getErrorMessage(error)}`)
    }
}
...
const updateUser = async (userId: string, requestBody: any): Promise<any> => {
    try {
        ...
        throw new InternalError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: userService.updateUser - ${getErrorMessage(error)}`)
    }
}
...
const validateUser = async (email: string, password: string): Promise<any> => {
    try {
        ...
        console.log(user)
        if (!user) {
            throw new InternalError(StatusCodes.UNAUTHORIZED, `Error: userService.valdiateUser - Unauthorized access`)
        }
